...
try {
	...
	await section('flakes', main, async () => {
		...
		await page.mouse.move(cx, cy)
		check('hovering your flake shows its tooltip', (await text(page, '#tip')) === `Your oat flake near ${nearName} · not reached yet`, await text(page, '#tip'))
		await page.mouse.down()
		await page.mouse.up()
		await settle(page)
		check('clicking a flake takes it away (HUD −1)', (await hud(page)) === `1 of ${n0} oat flakes`, await hud(page))
		const took = (await log(page))[0]
		check('…and logs that you took it away', took?.text === `You took away your oat flake near ${nearName}` && took?.you, JSON.stringify(took))
