// Renders tests/render-demo.html at a few moments of growth, light and dark,
// at 1x and 2x device pixels, plus a few close-ups through the hand lens,
// and writes PNGs to tests/output/render/.
//
...
const STEPS = (process.env.STEPS || '150,600,1500,3000').split(',').map(Number).filter((n) => n > 0).sort((a, b) => a - b)
...
const save = async (name, shot) => {
	...
	const s = shot.stats
	console.log(`  ${name}: ${Math.round(shot.ms)} ms${s ? `, ${s.tubes} tubes, ${s.veins} veins, ~${(s.tubeFragments / 1e6).toFixed(2)} M tube px, cpu ${s.cpuMs.toFixed(2)} ms` : ''}`)
}
...
try {
	...
	for (const target of STEPS) {
		...
		for (const dark of [false, true]) {
			for (const dpr of [1, 2]) {
				...
				await save(`dish-${pad(done)}-${dark ? 'dark' : 'light'}@${dpr}x.png`, shot)
			}
			// close-ups at 2x: the hand lens on the middle of the city, and on CODE
			...
			for (const [name, view] of [
				...
				await save(`lens-${pad(done)}-${name}-${dark ? 'dark' : 'light'}.png`, shot)
			}
		}
	}
	...
	for (const dark of [false, true]) {
		...
		await save(`dish-${pad(done)}-lamp-${dark ? 'dark' : 'light'}@1x.png`, shot)
	}
} finally {
	await browser.close()
	server.close()
